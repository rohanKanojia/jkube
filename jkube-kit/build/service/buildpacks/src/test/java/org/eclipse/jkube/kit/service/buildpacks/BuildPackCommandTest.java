...
class BuildPackCommandTest {
  ...
  void getArgs() {
    // Given
    BuildPackCommand buildPackCommand = new BuildPackCommand(kitLogger, "pack", Collections.singletonList("--version"), s -> {});
  ...
  void processLine_whenInvoked_shouldSetVersion() {
    ...
    BuildPackCommand buildPackCommand = new BuildPackCommand(kitLogger, "pack", Collections.singletonList("--version"), version::set);
    // When
